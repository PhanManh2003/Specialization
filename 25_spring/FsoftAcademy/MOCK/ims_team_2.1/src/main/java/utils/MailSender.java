...
public class MailSender {
    ...
    // 5. Send offer reminder email
    public void sendOfferReminderEmail(
            String senderMailAddress,
            String receiverMailAddress,
            String password,
            Candidate candidate,
            UserAccount recruiter,
            String offerDetailLink
    ) throws MessagingException {
        // Cấu hình SMTP cho Gmail
        Properties props = new Properties();
        props.put("mail.smtp.auth", "true");
        ...
        message.setFrom(new InternetAddress(senderMailAddress));
        message.addRecipient(Message.RecipientType.TO, new InternetAddress(receiverMailAddress));
        message.setSubject("(no-reply-email-IMS-system) Take action on Job offer");

        // Tạo nội dung HTML
        String emailContent = "<p>This email is from IMS system, </p>"
                + "<p>You have an offer to take action For Candidate " + candidate.getName()
                + "<p>position " + candidate.getPosition() + ", the contract is attached with this no-reply-email.</p>"
                + "<p>Please refer this link to take action: "
                + "<strong><a href=\"" + offerDetailLink + "\">offer detail</a></strong></p>"
                + "<p>If anything wrong, please reach-out recruiter " + recruiter.getEmail() + " We are so sorry for this inconvenience.</p>"
                + "<p>Thanks & Regards!</p>"
                + "IMS Team.";
        ...
        if (cvData == null || cvData.length == 0) {
            //System.out.println("Candidate CV is null or empty. Sending email without attachment.");
    ...
    //6. send candidate offer reminder.
    //6. send candidate offer reminder.
    public void sendCandidateOfferReminderEmail(
            String senderMailAddress,
            String receiverMailAddress,
            String password,
            Candidate candidate,
        ...
        message.setFrom(new InternetAddress(senderMailAddress));
        message.addRecipient(Message.RecipientType.TO, new InternetAddress(receiverMailAddress));
        message.setSubject("Offer reminder");

        // Nội dung HTML
        String emailContent
                = "<div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;'>"
                + "<div style='text-align: center; padding: 15px 0; background-color: #f8f9fa; border-bottom: 3px solid #4285f4;'>"
                + "<h2 style='color: #333; margin: 0;'>Offer of Employment</h2>"
                + "</div>"
                + "<div style='padding: 20px;'>"
                + "<p>Dear " + candidate.getName() + ",</p>"
                + "<p>Congratulations on your successful interview process! We are pleased to inform you that our management team would like to extend you a formal offer of employment.</p>"
                + "<div style='background-color: #f8f9fa; border-left: 4px solid #4285f4; padding: 15px; margin: 20px 0;'>"
                + "<h3 style='color: #4285f4; margin-top: 0;'>Offer Details</h3>"
                + "<table style='width: 100%; border-collapse: collapse;'>"
                + "<tr>"
                + "<td style='padding: 8px 0; width: 40%; color: #666;'><strong>Contract Type:</strong></td>"
                + "<td style='padding: 8px 0;'>" + offer.getContractType() + "</td>"
                + "</tr>"
                + "<tr>"
                + "<td style='padding: 8px 0; width: 40%; color: #666;'><strong>Contract Duration:</strong></td>"
                + "<td style='padding: 8px 0;'>" + DateTimeUtils.formatLocalDateTime(offer.getContractStartFrom())
                + " to " + DateTimeUtils.formatLocalDateTime(offer.getContractTo()) + "</td>"
                + "</tr>"
                + "<tr>"
                + "<td style='padding: 8px 0; width: 40%; color: #666;'><strong>Position:</strong></td>"
                + "<td style='padding: 8px 0;'>" + offer.getPosition() + " (" + offer.getLevel() + ")</td>"
                + "</tr>"
                + "<tr>"
                + "<td style='padding: 8px 0; width: 40%; color: #666;'><strong>Department:</strong></td>"
                + "<td style='padding: 8px 0;'>" + offer.getDepartment() + "</td>"
                + "</tr>"
                + "<tr>"
                + "<td style='padding: 8px 0; width: 40%; color: #666;'><strong>Basic Salary:</strong></td>"
                + "<td style='padding: 8px 0;'>" + offer.getBasicSalary() + "</td>"
                + "</tr>"
                + "</table>"
                + "</div>"
                + "<div style='background-color: #f9f2f4; border: 1px solid #ddd; border-radius: 4px; padding: 15px; margin: 20px 0;'>"
                + "<p style='color: #c7254e; font-weight: bold; margin: 0;'>Please respond with your decision regarding this offer by contacting our recruiter:</p>"
                + "<p style='margin: 10px 0 0 0;'>Email: <a href='mailto:" + recruiter.getEmail() + "' style='color: #4285f4; text-decoration: none;'>" + recruiter.getEmail() + "</a><br>"
                + "Phone: <a href='tel:" + recruiter.getPhone() + "' style='color: #4285f4; text-decoration: none;'>" + recruiter.getPhone() + "</a></p>"
                + "</div>"
                + "<p>We look forward to your positive response and to potentially welcoming you to our team.</p>"
                + "<p style='margin-top: 30px;'>Thanks & Regards,<br><strong>IMS Team</strong></p>"
                + "</div>"
                + "<div style='text-align: center; padding: 15px; background-color: #f8f9fa; border-top: 1px solid #e0e0e0; font-size: 12px; color: #666;'>"
