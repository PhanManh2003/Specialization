...
//    }
    ...
    public void switchStatusUser(int userID) {
        String sql = "UPDATE dbo.UserAccount SET [Status] = \n"
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            Logger.getLogger(UserAccountDAO.class.getName()).log(Level.SEVERE, "Error updating user", ex);
        }
    }
    ...
    public List<UserAccount> getUserExceptInterviewer() {
        List<UserAccount> users = new ArrayList<>();
        String sql = "SELECT * FROM dbo.UserAccount WHERE [role] <> 'Interviewer' ";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ResultSet rs = statement.executeQuery();
            while (rs.next()) {
                UserAccount user = new UserAccount();
    ...
    public List<UserAccount> getUserByRole(String role) {
        List<UserAccount> users = new ArrayList<>();
        String sql = "SELECT * FROM dbo.UserAccount WHERE [role] = ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
